...
test("admin can select Sunburst, confirm high quality, and save the default", async (t) => {
  ...
  try {
    ...
    fireEvent.click(view.getByRole("button", { name: /GPT Image 2.5 Sunburst/ }));
    const dialog = await view.findByRole("dialog");
    assert.match(dialog.textContent ?? "", /High quality/);
    assert.equal(requests.length, 0);
    fireEvent.click(view.getByRole("button", { name: "Change default" }));
    await waitFor(() => assert.ok(view.getByText("Saved.")));
    assert.deepEqual(requests, [{ modelId: "gpt-image-2.5-sunburst" }]);
