...
test("Flare sends all four references to each medium-quality shot and supports retry/polling", async (t) => {
  ...
  assert.equal(getModel(modelId)?.gptImageQuality, "medium");
  ...
  for (const [index, request] of requests.entries()) {
    ...
    assert.equal(request.input.quality, "medium");
  ...
  assert.equal(requests[4].input.quality, "medium");
