...
function SelfieStack() {
  return (
    <div className="tilted-stack" style={{ position: 'relative', height: 460, maxWidth: 340, margin: '0 auto' }}>
      {SELFIES.map((p, i) => (
        <figure
          key={p.cap}
          className="polaroid"
          style={{ position: 'absolute', width: 160, left: p.x, top: p.y, transform: `rotate(${p.r}deg)`, zIndex: i }}
        >
          {/* eslint-disable-next-line @next/next/no-img-element */}
          <img src={p.img} alt="" style={{ aspectRatio: '1 / 1', objectFit: 'cover' }} loading="eager" decoding="async" />
          <figcaption style={{ fontFamily: 'var(--font-mono)', fontSize: 11, fontStyle: 'normal', color: 'var(--ink-muted)' }}>
            {p.cap}
          </figcaption>
        </figure>
      ))}
...
function FlowArrow() {
  return (
    <div style={{ textAlign: 'center', padding: '0 8px' }}>
      <div className="flow-arrow" style={{ marginBottom: 14 }}>becomes</div>
      <svg width="120" height="32" viewBox="0 0 120 32" style={{ display: 'block', margin: '0 auto', color: 'var(--coral)' }}>
        <path d="M2 16 Q 30 4, 60 16 T 110 16" stroke="currentColor" strokeWidth="1.5" fill="none" strokeLinecap="round" strokeDasharray="2 5" />
        <path d="M104 9 L 113 16 L 104 23" stroke="currentColor" strokeWidth="1.5" fill="none" strokeLinecap="round" strokeLinejoin="round" />
      </svg>
      <div className="small-caps" style={{ marginTop: 14, color: 'var(--coral-deep)' }}>~ two minutes</div>
    </div>
  );
}
  ...
  return (
    ...
    >
      <div style={{ maxWidth: '76rem', margin: '0 auto', padding: '0 1.5rem' }}>
        ...
        <div style={{ display: 'flex', gap: 12, justifyContent: 'center', marginTop: 72, flexWrap: 'wrap' }}>
          <a href={ctaHref()} className="btn btn-coral btn-lg spring-press">Begin a card</a>
          <a href="https://familyshoot.com/#gallery" className="btn btn-ghost btn-lg spring-press">See the gallery</a>
        </div>
        <div className="small-caps" style={{ textAlign: 'center', marginTop: 22, color: 'var(--ink-muted)' }}>
