      ...
      <div className="mx-auto grid max-w-6xl gap-12 md:grid-cols-[1.2fr_1fr_1fr]">
        ...
        <div>
          ...
          <ul className="mt-4 space-y-2.5 text-sm text-[color:rgba(251,248,243,0.85)]">
            ...
            <li><a href="mailto:hi@familyphotoshoot.ai" className="hover:text-white transition-colors">Contact</a></li>
          </ul>
        </div>
      </div>

      <div className="mx-auto mt-16 max-w-6xl border-t border-[color:rgba(251,248,243,0.1)] pt-10">
        <nav aria-label="Browse all" className="flex flex-wrap items-center justify-center gap-2 sm:gap-3">
        ...
        <div className="mt-10 grid gap-10 md:grid-cols-3">
          <FooterLinkGroup title="Family portrait vibes" items={VIBES.map(v => ({ href: `/${v.slug}`, label: v.name }))} />
          <FooterLinkGroup title="Family photo cards" items={CARDS.map(c => ({ href: `/${c.slug}`, label: c.name }))} />
          <FooterLinkGroup title="Portrait art styles" items={STYLES.map(s => ({ href: `/${s.slug}`, label: s.name }))} />
        </div>
      </div>
...
function FooterLinkGroup({ title, items }: { title: string; items: { href: string; label: string }[] }) {
  return (
    <div>
...
function HubPill({ href, label, count }: { href: string; label: string; count: number }) {
  return (
    ...
    >
      <span>{label}</span>
      <span className="rounded-full bg-[color:rgba(251,248,243,0.1)] px-2 py-0.5 text-xs text-[color:rgba(251,248,243,0.65)]">{count}</span>
    </Link>
  );
}
